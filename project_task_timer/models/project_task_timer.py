    ...
    def toggle_start(self):
        for rec in self:
            if rec.task_timer is True:
                ...
                time_line = rec.env['account.analytic.line']
                for time_sheet in rec:
                    time_line.create({
                        'name': rec.env.user.name + ': ' + time_sheet.name,
                ...
                time_line_obj = rec.env['account.analytic.line']
                domain = [('task_id', 'in', rec.ids), ('date_end', '=', False)]
