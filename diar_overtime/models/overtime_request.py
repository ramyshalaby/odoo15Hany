...
class HrOverTime(models.Model):
    ...
    type = fields.Selection([('cash', 'Cash'), ('leave', 'leave')], default="leave", string="Type")
    ...
    timesheet_ids = fields.Many2many('account.analytic.line', string='Timesheet')
    ...
    request_type = fields.Selection([('overtime','Overtime'), ('parttime','Part Time')], 'Type')
    ...
    def create(self, values):
        if 'request_type' in values and values.get('request_type') == 'overtime':
            seq = self.env['ir.sequence'].next_by_code('hr.overtime') or '/'
        elif 'request_type' in values and values.get('request_type') == 'parttime':
            seq = self.env['ir.sequence'].next_by_code('hr.parttime') or '/'
        values['name'] = seq
    ...
    def _onchange_date(self):
        ...
        if self.sudo().contract_id and self.date_from and self.date_to:
            ...
            hr_timesheet = self.env['account.analytic.line'].search(
                [('date', '>=', self.date_from),
                 ('date', '<=', self.date_to),
                 ('project_id','=',self.project_id.id),
                 ('employee_id', '=', self.employee_id.id)])
            self.update({
                'timesheet_ids': [(6, 0, hr_timesheet.ids)]
            })
