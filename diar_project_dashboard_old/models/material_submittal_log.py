...
class material_submittal_log(models.Model):
    _name = 'material.submittal.log'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Material Submittal Log - Structural Works"
    ...
    material_submittal_log_details_ids = fields.One2many('material.submittal.log.details', 'material_submittal_log_id',
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].get('material.submittal.log') or ' '
        return super(material_submittal_log, self).create(vals)

    def write(self, vals):
        vals['last_update'] = fields.Datetime.now()
        return super(material_submittal_log, self).write(vals)
...
class material_submittal_log_details(models.Model):
    _name = 'material.submittal.log.details'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Material Inspection Request Log Details"

    material_submittal_log_id = fields.Many2one('material.submittal.log', 'Material Inspection Request Log')

    reference = fields.Char('material Submittal Ref.', track_visibility='always')
    ...
    material_sample = fields.Char("Material Sample", track_visibility='always')
    ...
    mirStatus = fields.Selection([('opened', 'Opened'), ('closed', 'Closed')], 'IR Status', track_visibility='always')
