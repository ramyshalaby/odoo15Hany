...
class document_submittal_sd_log(models.Model):
    _name = 'document.submittal.sd.log'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Document Submittal (SD) Log"
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].get('document.submittal.sd.log') or ' '
        return super(document_submittal_sd_log, self).create(vals)

    def write(self, vals):
        vals['last_update'] = fields.Datetime.now()
        return super(document_submittal_sd_log, self).write(vals)
...
class document_submittal_log_details(models.Model):
    _name = 'document.submittal.sd.log.details'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Document Submittal (SD) Log Details"

    document_submittal_sd_log_id = fields.Many2one('document.submittal.sd.log', 'Document Submittal SD Log')
    ...
    drawing_number = fields.Char("Drawing Number", track_visibility='always')
    drawing_rev = fields.Char("Drawing Review", track_visibility='always')
    ...
    Level = fields.Char("Level", track_visibility='always')
    drawing_title = fields.Char("Drawing Title", track_visibility='always')
    ...
    dos = fields.Selection([('latest', 'Latest'), ('superseded', 'Superseded')], 'Drawing', track_visibility='always')
    dosStatus = fields.Selection([('opened', 'Opened'), ('closed', 'Closed')], 'Drawing Status',
    ...
    def calc_replyduration(self):
        for record in self:
            ...
            if record.receiveddate < record.sentdate:
                raise ValidationError(_("Sorry send date can't exceed received date"))
            if record.sentdate and record.receiveddate and record.receiveddate > record.sentdate:
