...
class AttendanceWizard(models.TransientModel):
    ...
    def download_device_attendance(self):
        ...
        for rec in self.search():
            rec.cron_download_device_attendance()
    ...
    def cron_sync_attendance(self):
        ...
        for rec in self.search():
            rec.with_context(synch_ignore_constraints=True).sync_attendance()
    ...
    def sync_attendance(self):
        ...
        PlanningAttendance = self.env['planning.slot']
        ...
        for activity_id in activity_ids:
            ...
            for att in unsync_data:
                if att.employee_id.contract_id.multi_shift == True:

                        planning = PlanningAttendance.search([('employee_id', '=', att.employee_id.id),
                                                              ('device_start_date','<=',att.timestamp),('device_end_date','>=',att.timestamp),],
                                                             )
                         ...
                         for plan in planning:
                            ...
                            last_employee_attendance[activity_id.id][employee_id.id] = HrAttendance.search(
                                [('employee_id', '=', employee_id.id),
                                 ...
                                 ('check_in', '>=', plan.device_start_date),('check_in','<=',plan.device_end_date)], limit=1, order='check_in DESC')
                        ...
                        if hr_attendance_id.id != False:
                            if att.timestamp.date() == hr_attendance_id.check_in.date():
                                try:
